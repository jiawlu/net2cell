...
class CAutoConnectorM:
    ...
    def getLaneConnection(cls):
        ...
        ob_lanes = cls.ob_link.number_of_lanes


        left_ib_link = cls.ib_link_list_sorted[0]
        connection_lanes = min(ob_lanes, left_ib_link.number_of_lanes)
        cls.connection_list[cls.ib_link_list.index(left_ib_link)] = [(left_ib_link.number_of_lanes-connection_lanes, left_ib_link.number_of_lanes-1),
                                                                     (0, connection_lanes-1)]        # in, out

        for ib_link in cls.ib_link_list_sorted[1:]:
            connection_lanes = min(ob_lanes, ib_link.number_of_lanes)
            cls.connection_list[cls.ib_link_list.index(ib_link)] = [(0, connection_lanes - 1),
                                                                    (ob_lanes-connection_lanes, ob_lanes-1)]  # in, out

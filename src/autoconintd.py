...
# @desc         [auto connection for intersection and diverge]
...
class CAutoConnectorIntD:
    ...
    def getLaneConnection(cls):
        ...
        ib_lanes = cls.ib_link.number_of_lanes
        ...
        if ib_lanes == 1:
            ...
            for ob_link in cls.ob_link_list_sorted[1:]:
                cls.connection_list[cls.ob_link_list.index(ob_link)] = [(0, 0), (ob_link.number_of_lanes-1, ob_link.number_of_lanes-1)]
            return

        if len(cls.ob_link_list_sorted) == 1:
            # only one ob_link, full connection
            ob_link = cls.ob_link_list_sorted[0]
            connection_lanes = min(ib_lanes, ob_link.number_of_lanes)
            ...
            lanes_ob_has = left_ob_link.number_of_lanes
            ...
            cls.connection_list[cls.ob_link_list.index(right_ob_link)] = [
                ...
                (right_ob_link.number_of_lanes - cls.default_right_most_lanes, right_ob_link.number_of_lanes - 1)]
            ...
            lanes_each_mid_ob_link_left = [link.number_of_lanes for link in mid_ob_link_list]
            ...
            if ib_lanes - cls.default_left_most_lanes - cls.default_right_most_lanes >= len(mid_ob_link_list):
                ...
                for ob_link_no, ob_link in enumerate(mid_ob_link_list):
                    cls.connection_list[cls.ob_link_list.index(ob_link)] = [
                        ...
                        (ob_link.number_of_lanes - lanes_assgined_to_each_mid_ob_link[ob_link_no], ob_link.number_of_lanes - 1)]
                ...
                for lane_no in range(ib_lanes-1):
                    ...
                    cls.connection_list[cls.ob_link_list.index(ob_link)] = [(lane_no, lane_no), (ob_link.number_of_lanes - 1, ob_link.number_of_lanes - 1)]
                lane_no += 1
                ...
                for ob_link in mid_ob_link_list:
                    cls.connection_list[cls.ob_link_list.index(ob_link)] = [(start_lane_no, start_lane_no), (ob_link.number_of_lanes - 1, ob_link.number_of_lanes - 1)]
                    start_lane_no += 1
